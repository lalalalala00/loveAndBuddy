...
export const kOf = (c: {name?: string; issuer?: string; acquired_at?: string|null}) =>
  `${norm(c.name)}|${norm(c.issuer)}|${c.acquired_at ?? ''}`;

// 기존 + 새 입력 합치고 dedupe (기존 id 보존)
export function mergeAndDedupeCerts(
  ...
  // 2) 새로 입력한 것 합치되, 키 충돌시 기존(id 있는) 것을 우선
