...
const filterPresets = {
  ...
  etherealTomato:
    "brightness(1.25) blur(0.2px) contrast(0.85) sepia(0.15) hue-rotate(-5deg) saturate(1.05)",
};
...
const EditorBox = () => {
  ...
  const computedFilter = useMemo(() => {
    if (filterMode === "preset") {
      ...
      const exposureFilter = `brightness(${exposure}%) contrast(${
        200 - exposure
      }%)`;
      return {
        filter: `
      ...
      <div className="flex justify-between">
        <div className="relative  max-h-[1200px] flex items-center justify-center">
          {imageUrl || imageUrls.length > 0 ? (
            ...
            <span className="text-gray-400 min-w-[800px]">
              이미지를 업로드하세요
            </span>
          )}
        </div>
